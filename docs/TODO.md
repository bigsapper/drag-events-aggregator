...
- [ ] Add per-site configuration support in `src/drag_events/config/sources.json` for rate limits, custom headers, and pagination depth (currently hardcoded constants)
- [ ] Add an `enabled` flag to `src/drag_events/config/tracks.json` and `src/drag_events/config/sources.json` entries so individual sites can be disabled without editing code
- [ ] Add schema validation for `src/drag_events/config/tracks.json` and `src/drag_events/config/sources.json` at startup so malformed entries fail fast with a clear error
