...
def test_process_flyer_duplicate(tmp_path, sample_events):
    img = make_1x1_png(tmp_path / "flyer.jpg")
    with patch("drag_events.flyer_processing.compute_phash", return_value="aabbccdd00112233"), \
         patch("drag_events.flyer_processing.is_duplicate_image", return_value=sample_events[0]):
...
def test_process_flyer_new_event(tmp_path, sample_events, mock_vision_client, sample_extracted):
    img = make_1x1_png(tmp_path / "flyer.jpg")
    with patch("drag_events.flyer_processing.compute_phash", return_value="00000000ffffffff"), \
         patch("drag_events.flyer_processing.is_duplicate_image", return_value=None), \
         patch("drag_events.flyer_processing.find_same_event", return_value=None):
        outcome, event = flyer_processing.process_flyer(str(img), sample_events)
    assert outcome == "new"
    assert "id" in event
...
def test_process_flyer_normalizes_contact_website(tmp_path, sample_events):
    ...
    with patch("drag_events.flyer_processing.compute_phash", return_value="00000000ffffffff"), \
         patch("drag_events.flyer_processing.is_duplicate_image", return_value=None), \
         patch("drag_events.flyer_processing.extract_event", return_value=extracted), \
         patch("drag_events.flyer_processing.find_same_event", return_value=None):
        _, event = flyer_processing.process_flyer(str(img), sample_events)
...
def test_process_flyer_merged(tmp_path, sample_events, mock_vision_client, sample_extracted):
    ...
    with patch("drag_events.flyer_processing.compute_phash", return_value="00000000ffffffff"), \
         patch("drag_events.flyer_processing.is_duplicate_image", return_value=None), \
         patch("drag_events.flyer_processing.find_same_event", return_value=sample_events[0]):
        outcome, event = flyer_processing.process_flyer(str(img), sample_events)
...
def test_process_flyer_skips_out_of_scope_event(tmp_path, sample_events, mock_vision_client):
    ...
    with patch("drag_events.flyer_processing.compute_phash", return_value="00000000ffffffff"), \
         patch("drag_events.flyer_processing.is_duplicate_image", return_value=None), \
         patch("drag_events.flyer_processing.extract_event", return_value={"title": "2026 TMCCC Banquet", "track": {}, "dates": {"start": "2026-12-05"}}):
        outcome, event = flyer_processing.process_flyer(str(img), sample_events)
...
def test_process_flyer_skips_past_event(tmp_path, sample_events, mock_vision_client):
    ...
    with patch("drag_events.flyer_processing.compute_phash", return_value="00000000ffffffff"), \
         patch("drag_events.flyer_processing.is_duplicate_image", return_value=None), \
         patch("drag_events.flyer_processing.extract_event", return_value={"title": "Old Flyer", "track": {}, "dates": {"start": "2025-01-01"}}):
        outcome, event = flyer_processing.process_flyer(str(img), sample_events)
    assert outcome == "skipped"
...
def test_main_processes_image_and_saves(tmp_path, tmp_events_file, mock_vision_client, sample_extracted):
    ...
    with patch.object(sys, "argv", ["flyer_processing.py", str(img)]), \
         patch("drag_events.flyer_processing.compute_phash", return_value="00000000ffffffff"), \
         patch("drag_events.flyer_processing.is_duplicate_image", return_value=None), \
         patch("drag_events.flyer_processing.find_same_event", return_value=None):
        flyer_processing.main()
...
def test_main_counts_errors(tmp_path, tmp_events_file, capsys):
    img = make_1x1_png(tmp_path / "flyer.jpg")
    with patch.object(sys, "argv", ["flyer_processing.py", str(img)]), \
         patch("drag_events.flyer_processing.compute_phash", side_effect=RuntimeError("boom")):
        flyer_processing.main()
