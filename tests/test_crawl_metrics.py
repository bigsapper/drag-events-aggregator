...
def test_log_error_writes_context_details_and_message(tmp_path):
    ...
    with patch("drag_events.crawl.cli.should_log_errors", return_value=True):
        crawl.log_error(
            "fetch_page",
            RuntimeError("boom"),
            error_log=error_log,
...
def test_log_error_uses_default_error_log_path_when_not_provided(tmp_path):
    ...
    with patch("drag_events.crawl.cli.should_log_errors", return_value=True), \
         patch("drag_events.crawl.cli.get_error_log_path", return_value=default_log):
...
def test_log_error_can_include_traceback_without_trailing_newline(tmp_path):
    ...
    try:
        ...
        with patch("drag_events.crawl.cli.should_log_errors", return_value=True), \
             patch("drag_events.crawl.cli.traceback.format_exc", return_value="TRACE"):
...
def test_log_error_skips_manual_test_flyer_errors(tmp_path):
    ...
    with patch.dict("drag_events.crawl.cli.os.environ", {}, clear=False):
        crawl.log_error(
...
def test_main_skips_runtime_metric_recording_under_pytest(monkeypatch):
    ...
    with patch.object(sys, "argv", ["crawl.py"]), \
         patch("drag_events.crawl.cli.json.loads", return_value=[]), \
         patch("drag_events.crawl.cli.run_extraction", return_value={}), \
         patch("drag_events.crawl.cli.save_state"), \
         patch("drag_events.crawl.cli.time.sleep"), \
         patch("drag_events.crawl.cli.record_run_metrics") as mock_record:
        crawl.main()
...
def test_main_metrics_mode_prints_summary_and_returns(monkeypatch):
    ...
    with patch.object(sys, "argv", ["crawl.py", "--metrics"]), \
         patch("drag_events.crawl.cli.load_metric_entries", return_value=[{"status": "success", "elapsed_seconds": 10}]) as mock_load, \
         patch("drag_events.crawl.cli.print_metrics_summary") as mock_print:
        crawl.main()
...
def test_main_records_metrics_outside_pytest_when_extraction_returns_non_dict(monkeypatch):
    ...
    with patch.object(sys, "argv", ["crawl.py"]), \
         patch("drag_events.crawl.cli.json.loads", return_value=[]), \
         patch("drag_events.crawl.cli.get_retry_telemetry", return_value={"http": {"retries": 1}, "claude": {"retries": 2}}), \
         patch("drag_events.crawl.cli.run_extraction", return_value="not-a-dict"), \
         patch("drag_events.crawl.cli.save_state"), \
         patch("drag_events.crawl.cli.time.sleep"), \
         patch("drag_events.crawl.cli.record_run_metrics", return_value={"average_seconds": 10, "median_seconds": 10}) as mock_record:
        crawl.main()
...
def test_main_resets_retry_telemetry_before_run(monkeypatch):
    ...
    with patch.object(sys, "argv", ["crawl.py"]), \
         patch("drag_events.crawl.cli.json.loads", return_value=[]), \
         patch("drag_events.crawl.cli.reset_retry_telemetry") as mock_reset, \
         patch("drag_events.crawl.cli.get_retry_telemetry", return_value={"http": {}, "claude": {}}), \
         patch("drag_events.crawl.cli.run_extraction", return_value={}), \
         patch("drag_events.crawl.cli.save_state"), \
         patch("drag_events.crawl.cli.time.sleep"), \
         patch("drag_events.crawl.cli.record_run_metrics", return_value={"average_seconds": 10, "median_seconds": 10}):
        crawl.main()
...
def test_main_logs_and_reraises_top_level_error_outside_pytest(monkeypatch):
    ...
    with patch.object(sys, "argv", ["crawl.py"]), \
         patch("drag_events.crawl.cli.load_state", side_effect=RuntimeError("boom")), \
         patch("drag_events.crawl.cli.log_error") as mock_log, \
         patch("drag_events.crawl.cli.record_run_metrics", return_value={}) as mock_record, \
         patch("drag_events.crawl.cli.time.sleep"):
        try:
            crawl.main()
        except RuntimeError as exc:
